...
test "runtime: struct field returns value" {
    const source = @embedFile("fixtures/runtime/struct_field.veyl");
    ...
    const result = try vm.runFirst(&bytecode);
    try std.testing.expectEqualStrings("Ada", result.string);
}

test "runtime: print builtin returns unit" {
    const source = @embedFile("fixtures/runtime/print_builtin.veyl");
