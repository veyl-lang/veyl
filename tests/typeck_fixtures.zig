...
test "typeck diagnostic: struct field value mismatch" {
    const source = @embedFile("fixtures/typeck/struct_field_type_mismatch.veyl");
    const expected = @embedFile("fixtures/typeck/struct_field_type_mismatch.diag");

    var sources = veyl.base.SourceMap.init(std.testing.allocator);
    defer sources.deinit();
    const source_id = try sources.add("struct_field_type_mismatch.veyl", source);
